...
if( !defined( 'MP_STACKS_MAILCHIMP_VERSION' ) )
	define( 'MP_STACKS_MAILCHIMP_VERSION', '1.0.0.4' );

// Plugin Folder URL
if( !defined( 'MP_STACKS_MAILCHIMP_PLUGIN_URL' ) )
	define( 'MP_STACKS_MAILCHIMP_PLUGIN_URL', plugin_dir_url( __FILE__ ) );

// Plugin Folder Path
if( !defined( 'MP_STACKS_MAILCHIMP_PLUGIN_DIR' ) )
	define( 'MP_STACKS_MAILCHIMP_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );

// Plugin Root File
if( !defined( 'MP_STACKS_MAILCHIMP_PLUGIN_FILE' ) )
	define( 'MP_STACKS_MAILCHIMP_PLUGIN_FILE', __FILE__ );
...
function mp_stacks_mailchimp_textdomain() {

	// Set filter for plugin's languages directory
	$mp_stacks_mailchimp_lang_dir = dirname( plugin_basename( MP_STACKS_MAILCHIMP_PLUGIN_FILE ) ) . '/languages/';
	$mp_stacks_mailchimp_lang_dir = apply_filters( 'mp_stacks_mailchimp_languages_directory', $mp_stacks_mailchimp_lang_dir );


	// Traditional WordPress plugin locale filter
	$locale        = apply_filters( 'plugin_locale',  get_locale(), 'mp-stacks-mailchimp' );
	$mofile        = sprintf( '%1$s-%2$s.mo', 'mp-stacks-mailchimp', $locale );

	// Setup paths to current locale file
	$mofile_local  = $mp_stacks_mailchimp_lang_dir . $mofile;
	$mofile_global = WP_LANG_DIR . '/mp-stacks-mailchimp/' . $mofile;

	if ( file_exists( $mofile_global ) ) {
		// Look in global /wp-content/languages/mp-stacks-mailchimp folder
		load_textdomain( 'mp_stacks_mailchimp', $mofile_global );
	} elseif ( file_exists( $mofile_local ) ) {
		// Look in local /wp-content/plugins/mp-stacks-mailchimp/languages/ folder
		load_textdomain( 'mp_stacks_mailchimp', $mofile_local );
	} else {
		// Load the default language files
		load_plugin_textdomain( 'mp_stacks_mailchimp', false, $mp_stacks_mailchimp_lang_dir );
	}

}
add_action( 'init', 'mp_stacks_mailchimp_textdomain', 1 );
...
|--------------------------------------------------------------------------
*/
function mp_stacks_mailchimp_include_files(){
	/**
